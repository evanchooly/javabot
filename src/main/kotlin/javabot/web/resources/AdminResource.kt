...
constructor(
    ...
    fun configureRoutes(routing: Routing) {
        routing {
            route("/admin") {
                get {
                    val user = getAuthenticatedUser(call) ?: run {
                    ...
                    if (current == null) {
                        ...
                        val view = viewFactory.createAdminIndexView(KtorServletRequest(call), current, Admin())
                ...
                get("/config") {
                    ...
                    adminDao.getAdminByEmailAddress(user.email) ?: run {
                        call.respond(HttpStatusCode.Forbidden)
                        return@get
                    }
                ...
                get("/editChannel/{channel}") {
                    ...
                    val channelName = call.parameters["channel"] ?: run {
                    ...
                    val channel = channelDao.get(channelName) ?: run {
                ...
                post("/saveConfig") {
                    ...
                    config.historyLength = params["historyLength"]?.toIntOrNull() ?: config.historyLength
                    ...
                    config.throttleThreshold = params["throttleThreshold"]?.toIntOrNull() ?: config.throttleThreshold
                    config.minimumNickServAge = params["minimumNickServAge"]?.toIntOrNull() ?: config.minimumNickServAge
                ...
                get("/edit/{id}") {
                    ...
                    val current = adminDao.getAdminByEmailAddress(user.email) ?: run {
                        call.respond(HttpStatusCode.Forbidden)
                        return@get
                    }

                    val view = viewFactory.createAdminIndexView(KtorServletRequest(call), current, adminDao.find(ObjectId(id)))
                    call.respond(FreeMarkerContent(view.getChildView(), view.toModel()))
                }
                ...
                post("/addApi") {
                    ...
                    version?.let {
                        val apiName = name ?: artifactId ?: run {
                            call.respond(HttpStatusCode.BadRequest)
                            return@post
                        }
