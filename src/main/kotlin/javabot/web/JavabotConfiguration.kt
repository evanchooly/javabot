...
class JavabotConfiguration {
    ...
    @JsonProperty var OAuthSuccessUrl: String? = null

    @JsonProperty private val oauthCustomCfg: HashMap<String, String>? = null

    fun getOAuthCfgProperties(): Properties {
        val properties = Properties()
        for (oauth in OAuthCfg!!) {
            properties.put(oauth.prefix + ".consumer_key", oauth.key)
            properties.put(oauth.prefix + ".consumer_secret", oauth.secret)
            if (oauth.permissions != null) {
                properties.put(oauth.prefix + ".custom_permissions", oauth.permissions)
            }
        }
