...
/**
 * Implements a weather service Dao using OpenWeather's weather API. We may need to eventually think
 * about OWM's terms of service; the free key allows only 1000 requests a day. This may not be a
 * problem in normal usage, but what if a malicious bot or user issues 4000 weather requests? They'd
 * get denied (the API would reject them) but it's still impolite behavior on the part of the bot.
 *
...
class OpenWeatherMapHandler(
    ...
    @Suppress("UnstableApiUsage") private val rateLimiter = RateLimiter.create(60.0)
        ...
        return if (apiKey.isNotEmpty()) {
            if (rateLimiter.tryAcquire()) {
                ...
                try {
                    ...
                    // will throw an exception if the lat/long is empty. We WANT this. It'll exit
                    // cleanly.
